...
with models.DAG(
    ...
    description="Demonstrates the use of various Dataprep operators.",
    tags=["demo", "google_cloud", "dataprep"],
) as dag:
    create_bucket_task = GCSCreateBucketOperator(
        task_id="create_bucket", bucket_name=GCS_BUCKET_NAME, project_id=PROJECT_ID
    )
    ...
    def create_imported_dataset():
        ...
        response = hook.create_imported_dataset(
            body_request={"uri": DATASET_URI, "name": DATASET_NAME}
        )
        return response
